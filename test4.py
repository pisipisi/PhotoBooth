...
import time, os, io, subprocess, Image, shutil, cups, sys, pygame, piggyphoto
...
def capturePhoto():
    ...
    time.sleep(0.7)
    return photo 
def showPreview(file):
    picture = pygame.image.load(file) 
    picture = pygame.transform.scale(picture, (GRID_W_PX * preview_width, GRID_H_PX * preview_height))
    #preview_border = pygame.Surface((GRID_W_PX * preview_width, GRID_H_PX * preview_height))
    #preview_border.fill(BLACK)
    return DISPLAYSURF.blit(picture,(GRID_W_PX * preview_x, GRID_H_PX * preview_y))
def showPosePreView(file):
    picture = pygame.image.load(file)  
    picture = pygame.transform.scale(picture, (GRID_W_PX * preview_width, GRID_H_PX * preview_height))
    return DISPLAYSURF.blit(picture,(WINDOWWIDTH - (GRID_W_PX * preview_width) - (GRID_W_PX * p_preview_x), WINDOWHEIGHT/2 - (GRID_H_PX * preview_height)/2 - (GRID_H_PX * p_preview_y)))
...
def idleScreen():
    ...
    border.fill(BLACK)
    #DISPLAYSURF.blit(image, (GRID_W_PX * (preview_x + preview_pad),GRID_H_PX * (preview_y + preview_pad)))
    ...
    startRect.midbottom = (borderRect[2]/2+borderRect[0],borderRect[3]+borderRect[1]+50)
    ...
    while not GPIO.input(18) == False:
       ...
       # pygame.display.update(showPreview("preview.jpg"))
        FPSCLOCK.tick(FPS) 
...
def photoShoot():
    ...
    for photo in range (0,3):
        ...
        for i in range (5,0,-1):
            ...
            C.capture_preview('preview.jpg')
            pygame.display.update(showPosePreView("preview.jpg"))
            numSurf, numRect = makeTextObjs(str(i), HUGEFONT, WHITE)
            numRect.center = (GRID_W_PX*15,WINDOWHEIGHT/2- GRID_H_PX)
